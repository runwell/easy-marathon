...
export class GarminPlatform {
  ...
  async authenticate(credentials) {
    ...
    try {
      ...
      const sessionToken = this.base64EncodeUtf8(
        JSON.stringify({
          platform: 'garmin',
          credentials: {
  ...
  async submitLogin(email, password, csrfData) {
    ...
    if (!ticketMatch) {
      ...
      if (
        responseText.includes('credentials are incorrect') ||
      ...
      throw new Error(
        'Login failed - no ticket received. Garmin may have updated their login flow.',
      );
    }
  ...
  async exchangeTicket(ticket) {
    ...
    const response = await fetch(`${GarminPlatform.MODERN_URL}/?ticket=${ticket}`, {
      headers: {
        'User-Agent':
    ...
    for (const [name, value] of this.cookies) {
      console.log(
        `  ${name}: ${value ? value.substring(0, 30) + '... (len:' + value.length + ')' : 'EMPTY'}`,
      );
    }
    ...
    const oauth2 =
      this.cookies.get('SESSIONID') || this.cookies.get('SESSION') || this.cookies.get('session');
    ...
    console.log(
      '  oauth1 (GARMIN-SSO-CUST-GUID):',
      oauth1 ? `${oauth1.substring(0, 20)}... (len:${oauth1.length})` : 'MISSING',
    );
    console.log(
      '  oauth2 (SESSIONID):',
      oauth2 ? `${oauth2.substring(0, 20)}... (len:${oauth2.length})` : 'MISSING',
    );
    console.log(
      '  JWT_FGP:',
      jwtFgp ? `${jwtFgp.substring(0, 20)}... (len:${jwtFgp.length})` : 'MISSING',
    );
    console.log(
      '  JWT_WEB:',
      jwtWeb ? `${jwtWeb.substring(0, 20)}... (len:${jwtWeb.length})` : 'MISSING',
    );
  ...
  async getOAuthTokens(ticket) {
    const preauthUrl =
      `${GarminPlatform.CONNECT_API_URL}/oauth-service/oauth/preauthorized` +
    ...
    if (!exchangeResponse.ok) {
      ...
      throw new Error(
        `OAuth exchange failed: HTTP ${exchangeResponse.status} ${errorText.substring(0, 200)}`,
      );
    }
  ...
  parseSetCookiesVerbose(response, context) {
    ...
    if (typeof response.headers.getSetCookie === 'function') {
      ...
      const garminCookies = cookies.filter(
        (c) => c.includes('GARMIN') || c.includes('JWT') || c.includes('SESSIONID'),
      );
  ...
  parseSingleCookieVerbose(cookieString, context) {
    ...
    if (eqIndex > 0) {
      ...
      if (isImportant) {
        console.log(
          `[${context}] *** IMPORTANT COOKIE: ${name} = "${value}" (len: ${value.length}) ***`,
        );
      }
      ...
      if (
        value &&
        name &&
        !name.toLowerCase().startsWith('path') &&
        !name.toLowerCase().startsWith('domain')
      ) {
        console.log(
          `[${context}] Storing: ${name} = ${value.substring(0, 50)}${value.length > 50 ? '...' : ''} (len: ${value.length})`,
        );
        this.cookies.set(name, value);
      } else if (isImportant) {
  ...
  async fetchActivities(credentials, startDate, endDate, debugMode = false) {
    ...
    if (accessToken) {
      ...
      requests.push(
        ...
        {
          url: `${GarminPlatform.CONNECT_URL}/activitylist-service/activities?${params}`,
          headers: authHeaders,
        },
      );
  ...
  buildCookieString(credentials) {
    ...
    if (
      credentials.oauth1 &&
      typeof credentials.oauth1 === 'string' &&
      !addedCookies.has('GARMIN-SSO-CUST-GUID')
    ) {
  ...
  getOAuthAccessToken(credentials) {
    ...
    if (credentials.oauth2AccessToken) {
      if (credentials.oauth2ExpiresAt) {
        const expiresAtMs =
          typeof credentials.oauth2ExpiresAt === 'number'
            ? credentials.oauth2ExpiresAt > 1e12
              ? credentials.oauth2ExpiresAt
              : credentials.oauth2ExpiresAt * 1000
    ...
    if (
      credentials.oauth2 &&
      typeof credentials.oauth2 === 'object' &&
  ...
  normalizeActivities(rawActivities) {
    ...
    return rawActivities
      .filter((activity) => {
        ...
        const type =
          activity.activityType?.typeKey?.toLowerCase() ||
      ...
      .map((activity) => ({
        ...
        type: this.mapActivityType(
          activity.activityType?.typeKey || activity.typeKey || activity.type,
        ),
